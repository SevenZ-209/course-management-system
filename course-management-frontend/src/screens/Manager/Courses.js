...
const Courses = () => {
    ...
    const loadCourses = async () => {
        try {
            ...
            const res = await authApis().get(endpoints.managerCourses, { params });
            const data = res.data;
    ...
    const loadCategories = async () => {
        try {
            const res = await authApis().get(endpoints.managerCategoryOptions);
    ...
    const saveCourse = async e => {
        ...
        try {
            ...
            if (editingCourse)
                res = await authApis().put(`${endpoints.managerCourses}/${getCourseId(editingCourse)}`, data);
            else
                res = await authApis().post(endpoints.managerCourses, data);
    ...
    const updateStatus = async (courseId, newStatus) => {
        try {
            ...
            const res = await authApis().patch(
                `${endpoints.managerCourses}/${courseId}/status`,
                { status: newStatus }
            );
