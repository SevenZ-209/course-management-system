    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const res = await authApis().get(endpoints.managerReports, { params });
                setData(res.data || {});
            } catch(ex) {
                console.error("Load manager report error:", ex);
                setErr(ex.response?.data?.message || "Không thể tải báo cáo vận hành!");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <>
            <div className="cm-report-heading mb-4">
                <div>
                    <h2 className="fw-bold mb-1">Báo cáo vận hành</h2>
                    <p className="text-muted mb-0">Tổng hợp thanh toán, đăng ký học và tiến độ theo khoảng thời gian.</p>
                </div>
            </div>
