...
const OnlineSessions = () => {
    ...
    const loadSessions = async () => {
        try {
            ...
            const res = await authApis().get(endpoints.managerOnlineSessions, { params });
            const data = res.data;
    ...
    const loadCourses = async () => {
        try {
            const res = await authApis().get(endpoints.managerCourseOptions);
        ...
        try {
            ...
            if (editingSession)
                res = await authApis().put(
                    `${endpoints.managerOnlineSessions}/${getSessionId(editingSession)}`,
                    body
                );
            else
                res = await authApis().post(endpoints.managerOnlineSessions, body);
