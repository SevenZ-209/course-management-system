...
const Classes = () => {
    ...
    const loadClasses = async () => {
        try {
            ...
            const res = await authApis().get(endpoints.adminClasses, { params });
            const data = res.data;
    ...
    const saveClass = async e => {
        ...
        try {
            ...
            if (editingClass)
                res = await authApis().put(`${endpoints.adminClasses}/${getClassId(editingClass)}`, body);
            else
                res = await authApis().post(endpoints.adminClasses, body);
    ...
    const updateStatus = async (classId, newStatus) => {
        try {
            setErr("");
            setSuccess("");

            const res = await authApis().patch(
                `${endpoints.adminClasses}/${classId}/status`,
                { status: newStatus }
            );
            ...
            console.error("Update class status error:", ex);
