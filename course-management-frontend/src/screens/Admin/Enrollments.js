...
const Enrollments = () => {
    ...
    const loadEnrollments = async () => {
        try {
            ...
            const res = await authApis().get(endpoints.adminEnrollments, { params });
            const data = res.data;
    ...
    const saveEnrollment = async e => {
        ...
        try {
            ...
            const res = await authApis().post(endpoints.adminEnrollments, {
    ...
    const updateStatus = async (enrollmentId, newStatus) => {
        try {
            ...
            const res = await authApis().patch(
                `${endpoints.adminEnrollments}/${enrollmentId}/status`,
                { status: newStatus }
            );
            ...
            console.error("Update enrollment status error:", ex);
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p className="text-muted mb-0">
                        Tổng cộng <strong>{totalRecords}</strong> đăng ký
                    </p>
                </div>
                            ...
                            <Table hover className="align-middle mb-0">
                                <thead className="table-light">
                                    <tr>
                                        ...
                                        <th>Lớp học</th>
                                        <th>Trạng thái</th>
                                        <th style={{ minWidth: 170 }}>Cập nhật</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    {enrollments.map(e => (
                                        <tr key={getEnrollmentId(e)}>
                                            ...
                                            <td>
                                                <Form.Select size="sm" value={e.status}
                                                    onChange={event =>
                                                        updateStatus(getEnrollmentId(e), event.target.value)
                                                    }>
                                                    <option value="PENDING_PAYMENT">Chờ thanh toán</option>
                                                    <option value="ACTIVE">Đang hoạt động</option>
                                                    <option value="CANCELED">Đã hủy</option>
                ...
                <Form onSubmit={saveEnrollment}>
                    ...
                    <Modal.Body>
                        ...
                        <Form.Text className="text-muted">
                            Đăng ký mới sẽ có trạng thái Chờ thanh toán.
                        </Form.Text>
                    </Modal.Body>
