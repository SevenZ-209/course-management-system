...
public class ApiManagerAttendanceController {
    ...
    public AdminAttendancePageResponse getAttendances(
        ...
        return new AdminAttendancePageResponse(
                attendanceService.getAttendances(params).stream().map(attendanceMapper::toResponse).toList(),
                page, totalPages, totalRecords
        );
    }

    @PostMapping
    public AdminAttendanceActionResponse addAttendance(@RequestBody CreateAttendanceRequest request) {
        ...
        Enrollment enrollment = enrollmentService.getEnrollment(
                student.getId(), session.getCourseClass().getId()
        );
        if(enrollment == null || enrollment.getStatus() != Enrollment.EnrollmentStatus.ACTIVE)
            throw new IllegalArgumentException("Học viên không thuộc lớp học của buổi này!");
        if(attendanceService.getAttendance(session.getId(), student.getId()) != null)
            throw new IllegalArgumentException("Học viên đã có dữ liệu điểm danh cho buổi này!");

        Attendance attendance = new Attendance();
        attendance.setOnlineSession(session);
        attendance.setStudent(student);
        attendance.setPresent(request.present());
        attendance.setNote(normalizeNote(request.note()));
        attendanceService.addAttendance(attendance);

        return new AdminAttendanceActionResponse(attendance.getId(), "Điểm danh thành công!");
    }
    ...
    public List<AdminAttendanceStudentOptionResponse> getStudentsBySession(
        ...
        return enrollmentService.getActiveEnrollmentsByClass(session.getCourseClass().getId())
                .stream()
                .map(enrollment -> {
    ...
    private Attendance requireAttendance(Integer attendanceId) {
        ...
        if(attendance == null) throw new IllegalArgumentException("Không tìm thấy dữ liệu điểm danh!");
        return attendance;
    }

    private OnlineSession requireSession(Integer sessionId) {
        if(sessionId == null) throw new IllegalArgumentException("Vui lòng chọn buổi học!");
        OnlineSession session = sessionService.getSessionById(sessionId);
        if(session == null) throw new IllegalArgumentException("Buổi học không tồn tại!");
        return session;
    }
    ...
    private User requireStudent(Integer studentId) {
        ...
        if(student == null || student.getRole() != User.UserRole.STUDENT
            ...
            throw new IllegalArgumentException("Học viên không hợp lệ!");
        return student;
    }
}
