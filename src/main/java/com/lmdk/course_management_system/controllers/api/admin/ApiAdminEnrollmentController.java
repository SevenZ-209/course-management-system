...
public class ApiAdminEnrollmentController {

    private final EnrollmentService enrollmentService;
    private final CourseClassService classService;
    private final UserService userService;
    private final AdminEnrollmentMapper adminEnrollmentMapper;
    ...
    public AdminEnrollmentPageResponse getEnrollments(
        ...
        long totalRecords =
                enrollmentService.countEnrollments(params);
        ...
        return new AdminEnrollmentPageResponse(
                enrollmentService.getEnrollments(params)
                        .stream()
                        .map(adminEnrollmentMapper::toResponse)
                        .toList(),
                page,
                totalPages,
                totalRecords
    ...
    @PostMapping
    public AdminEnrollmentActionResponse addEnrollment(
            @RequestBody CreateAdminEnrollmentRequest request
    ) {
    ...
    public AdminEnrollmentActionResponse updateStatus(
        ...
        try {
            enrollment.setStatus(
                    Enrollment.EnrollmentStatus.valueOf(
    ...
    private User requireStudent(Integer studentId) {
        ...
        if(student.getRole() != User.UserRole.STUDENT)
            throw new IllegalArgumentException(
                    "Người dùng không phải học viên!"
            );
