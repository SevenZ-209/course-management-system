...
public class ApiManagerCourseController {
    ...
    public AdminCoursePageResponse getCourses(
        ...
        return new AdminCoursePageResponse(
                courseService.getCourses(params).stream().map(courseMapper::toResponse).toList(),
                page, totalPages, totalRecords
        );
    }

    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public AdminCourseActionResponse addCourse(@ModelAttribute CourseRequest request) {
        ...
        try {
            ...
            return new AdminCourseActionResponse(course.getId(), "Thêm khóa học thành công!");
        } catch(RuntimeException ex) {
            if(upload != null) cloudinaryService.deleteCourseImage(upload.publicId());
            throw ex;
        }
    }
    ...
    public AdminCourseActionResponse updateCourse(
        ...
        try {
            ...
            return new AdminCourseActionResponse(courseId, "Cập nhật khóa học thành công!");
        } catch(RuntimeException ex) {
            if(newUpload != null) {
    ...
    public AdminCourseActionResponse updateStatus(
        ...
        try {
            course.setStatus(Course.CourseStatus.valueOf(request.status().trim().toUpperCase()));
        } catch(IllegalArgumentException ex) {
            throw new IllegalArgumentException("Trạng thái không hợp lệ!");
        }

        courseService.updateCourse(course);
        return new AdminCourseActionResponse(courseId, "Cập nhật trạng thái thành công!");
    }
    ...
    private Category requireCategory(Integer categoryId) {
        if(categoryId == null) throw new IllegalArgumentException("Vui lòng chọn danh mục!");
